			...
			<table data-tablename="<?php echo $current_table; ?>" class="table table-striped table-hover table-condensed table-bordered">
				...
				<tbody>
					...
					<tr data-id="<?php echo html_attr($pkValue); ?>">
						<td class="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][1]}"; ?>" id="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][1]}-" . html_attr($record[$config['child-primary-key-index']]); ?>"><img src="<?php echo thisOr(get_embed('googlemap', $record[1], '', '', 'thumbnail_url'), 'blank.gif'); ?>" style="max-width: 50px; max-height: 50px; "></td>
					</tr>
					<?php } ?>
				</tbody>
				<tfoot>
